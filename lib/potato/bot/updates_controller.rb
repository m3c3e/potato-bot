...
module Potato
  module Bot
    ...
    #
    #     ControllerClass.new(bot, from: potato_user, chat: potato_chat).
    #       process(:help, *args)
    #
    class UpdatesController < AbstractController::Base # rubocop:disable ClassLength
      ...
      %w[
        ...
        session
      ].each { |file| require "potato/bot/updates_controller/#{file}" }

      %w[
        CallbackQueryContext
        MessageContext
        TypedUpdate
      ].each { |mod| autoload mod, "potato/bot/updates_controller/#{mod.underscore}" }
      ...
      ActiveSupport.run_load_hooks('potato.bot.updates_controller', self)
    end
  end
end

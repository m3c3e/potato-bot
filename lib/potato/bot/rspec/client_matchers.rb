module Potato
  module Bot
    module RSpec
      ...
      # Matchers to test requests to Potato API.
      #
      # Complex matchers requires `rspec-mocks` to be installed.
      module ClientMatchers
        class MakePotatoRequest < ::RSpec::Matchers::BuiltIn::BaseMatcher
          EXPECTATION_TYPES = {
            exactly: :==,
            at_most: :>=,
            at_least: :<=,
          ...
          def initialize(bot, action, description: nil)
            @bot = bot
            @action = action
            @description = description || "make #{action} potato request"
            exactly(1)
          end
        ...
        # Check that bot performed request to potato API:
        #
        #   expect { dispatch_message('Hi!') }.
        #     to make_potato_request(bot, :sendMessage).
        #     with(text: 'Hello!', chat_id: chat_id)
        def make_potato_request(bot, action)
          MakePotatoRequest.new(bot, action)
        end

        # Helper for asserting message is sent. Note that options are checked
        # with `hash_including`. For strict checks use #make_potato_request.
        def send_potato_message(bot, text = nil, options = {})
          description = "send potato message #{text.inspect}"
          text = a_string_matching(text) if text.is_a?(Regexp)
          options = options.merge(text: text) if text
          MakePotatoRequest.new(bot, :sendMessage, description: description).

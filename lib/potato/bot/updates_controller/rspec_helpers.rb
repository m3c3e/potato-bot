require 'potato/bot/updates_controller/testing'

RSpec.shared_context 'potato/bot/updates_controller' do
  let(:controller_class) { described_class }
  let(:controller) do
    controller_class.new(bot, update).tap do |x|
      x.extend Potato::Bot::UpdatesController::Testing
    end
  end
  let(:update) { build_update(payload_type, payload) }
  let(:payload_type) { :some_type }
  let(:payload) { double(:payload) }
  let(:bot) { Potato::Bot::ClientStub.new(bot_name) }
  let(:bot_name) { 'bot' }
...
RSpec.configure do |config|
  if config.respond_to?(:include_context)
    config.include_context 'potato/bot/updates_controller', type: :potato_bot_controller
  end
end

RSpec.describe Potato::Bot::ConfigMethods do
  let(:registry) do
    Object.new.tap do |x|
  ...
  describe '#bots_config' do
    ...
    context 'when not configured' do
      ...
      context 'in rails environment' do
        ...
        context 'when there is potato section in secrets' do
          let(:secrets) { {potato: config.stringify_keys} }
          let(:config) do
            {
          ...
          context 'on rails >5.1 (deep symbolized keys)' do
            let(:secrets) { {potato: config.deep_symbolize_keys} }
            it { should include default: config[:bot] }
            it { should include config[:bots] }
          end
        end

RSpec.describe Potato::Bot::Client do
  ...
  describe '#request' do
    ...
    around { |ex| Potato::Bot::ClientStub.stub_all!(false) { ex.run } }

    shared_examples 'invalid body' do |error = Potato::Bot::Error|
    ...
    context 'when status is 403' do
      let(:status) { 403 }
      it { should raise_error Potato::Bot::Forbidden, body_json['description'] }
      include_examples 'invalid body'
    end

    context 'when status is 404' do
      let(:status) { 404 }
      it { should raise_error Potato::Bot::NotFound, body_json['description'] }
      include_examples 'invalid body'
    end

    context 'when status is other' do
      let(:status) { 500 }
      it { should raise_error Potato::Bot::Error, /#{body_json['description']}/ }
    end
  end
end

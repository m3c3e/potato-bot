...
RSpec.describe 'Integrations helper', :potato_bot do
  include ActionDispatch::Integration::Runner
  def reset_template_assertion
  end

  let(:app) do
    app = Potato::Bot::Middleware.new(bot, controller)
  ...
  let(:bot) { Potato::Bot::ClientStub.new('token') }
  let(:controller_path) { '/' }
  let(:controller) do
  ...
  describe 'callback queries', :callback_query do
    let(:controller) do
      Class.new(Potato::Bot::UpdatesController) do
        include Potato::Bot::UpdatesController::CallbackQueryContext
